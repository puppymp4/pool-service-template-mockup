# Pool Builder Template — Rift Media

A premium React + Vite + Tailwind site template tuned for **pool builders** (custom design & construction — not pool service/cleaning).
...
1. **`src/App.tsx` · `BRAND` object** (top of file) — change every field: name, tagline, city, phone, email, address, license, hours, social, stats (pools/years/awards/rating).
...
4. **Hero imagery** — replace the Unsplash URL in `Hero` and the "Featured Build" card with the client's best shot (or a curated Unsplash image matching their aesthetic).
5. **`Portfolio` projects** — swap 6 `projects[]` entries. This section is the #1 conversion driver for builders, so real client photos > stock every time.
6. **`Testimonials` reviews** — pull 3 real Google reviews from the client.
7. **`ServiceArea` areas** — replace with the cities they actually serve.
8. **`TrustStrip` items** — replace marquee text with the client's actual certifications/awards.
9. **`FAQ` faqs** — price ranges, timelines, and financing answers should reflect the client's real numbers.
...
1. **Hero** — full-bleed imagery, serif headline, trust chip, dual CTA, featured-build glass card
2. **Trust strip** — marquee of certifications on ink-dark band
3. **Portfolio** — asymmetric grid, 2 large + 4 medium (the main selling tool)
4. **Process** — 4-step (Discover → Design → Build → Reveal) on ink-dark
5. **Services** — 6-card grid of build capabilities
6. **Testimonials** — 3-col pull-quotes with rating
7. **About** — studio positioning + stat tiles
8. **FAQ** — 6 questions, accordion
9. **Service Area** — pill cloud on ink-dark
10. **Contact** — consultation-request form (2 cols: intro + form)
